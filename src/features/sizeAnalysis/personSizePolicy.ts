...
/** 80~120 숫자 토큰 추출(120 포함, 집계·남90과 동일 정책) */
...
/**
 * 한 덩어리 문자열에서 성별 토큰 + 80~120 숫자가 모두 있으면 M100/W90/M120 등 및 수정완료(corrected).
...
export function tryFixedMwFromGenderAndNumericInString(text: string | undefined): PersonSizePolicyResult | null {
  ...
  if (line == null || num == null) return null;
  if (!NUMERIC_SIZES.has(num) && num !== "120") return null;
  const letter = line;
  ...
  return {
    ...
    parseReason:
      num === "120"
        ? "성별+숫자사이즈(120) 결합 → 표준 M/W 접두"
        : "성별+숫자사이즈(80~115) 결합 → 표준 M/W 접두",
  };
}
 ...
 * - 숫자만(80~115·120) → 남→M, 여→W 보정 → 수정완료(corrected) (120은 남90과 동일 집계·상태 톤)
...
export function normalizePersonSizePolicy(sizeRaw: string | undefined, genderRaw: string | undefined): PersonSizePolicyResult {
  ...
  if (m1) {
    ...
    const num = m1[2]!;
    if (NUMERIC_SIZES.has(num) || num === "120") {
      return {
        standardizedSize: `${letter}${num}`,
...
export function applyDuplicateSizePolicy(rows: NormalizedRow[], structureType: StructureType): NormalizedRow[] {
  ...
  function keyForRow(r: NormalizedRow): string | null {
    if (isMatrix) return duplicateGroupKeyFromRowWithSize(r);
  ...
  for (let i = 0; i < result.length; i += 1) {
    ...
    const excludeDetail = isMatrix
      ...
      : isMultiItem
        ? "same_club_same_name_same_item_same_size_keep_one"
