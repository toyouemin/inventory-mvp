...
export async function GET(req: Request) {
  ...
  if (debugCsv) {
    console.info("[csv/stock] fetched-counts", { fetchedProducts: list.length });
  }
  ...
  if (productIds.length > 0) {
    ...
    if (variantsErr) {
      return new Response(`Supabase error: ${variantsErr.message}`, { status: 500 });
    }
    variants = variantsRows;
    if (debugCsv) {
      console.info("[csv/stock] fetched-counts", {
  ...
  if (debugCsv) {
    console.info("[csv/stock] single-row-sku-summary", {
